...
This makes `git somnia-agent` command available as a Git subcommand.
...
```bash
cd /home/sharwin/GitFi/agent-template
git somnia-agent secrets set GROQ_API_KEY=sk-your-actual-groq-key-here
```
...
```bash
git somnia-agent secrets set AI_PROMPT="You are an aggressive degen trader. Should I 'BUY' or 'HOLD'?"
```
...
- [ ] Repository initialized with `somnia-agent init`
- [ ] Secrets set via CLI
