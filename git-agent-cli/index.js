...
program
  ...
  .action(async () => {
    ...
    console.log(`  2. Set your secrets: ${chalk.cyan('git somnia-agent secrets set GROQ_API_KEY=...')}`);
  ...
  .action(async () => {
    ...
    if (result && result.stats) {
      ...
      if (totalDecisions === 0) {
        ...
        console.log(chalk.yellow(`   → Check if the agent is running: ${chalk.cyan('git somnia-agent logs')}`));
